...
void ReadMCACe () {
    ...
    TH1D *hce = new TH1D("hce"," Sorgente Cesio - Scatterless - HV = -300V ",ch,0.,(double)ch);

    ifstream input("./Data/Sorgente Cesio [Scatterless]/beta-300Vgiusta.mca");
    ...
    while(input>>fillh) hce->SetBinContent(cntbin++,fillh); //legge i canali in ordine
    ...
    TFile *hfile = new TFile("hcesio.root","recreate");
    hce->Write();
