...
void AnalysisMCA_en () {
	...
	TF1* f1 = new TF1("func1","[0]*TMath::Erfc((x-[1])/(TMath::Sqrt(2)*[2]))+[3]",285,370);
	f1->SetParNames("Norm_{c}","E_{c}","#sigma_{c}","c");
...
hgamma[1]->Fit(f1,"RM+");
	...
	cout<<"Massa elettrone: "<< 2*511*(511-f1->GetParameter(1))/(f1->GetParameter(1)) << endl;
    	cout<<"Errore: " << 2*511*511/(f1->GetParameter(1)*f1->GetParameter(1))*f->GetParameter(2)<<endl;
    }
