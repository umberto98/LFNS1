\\Simulation of coincidence curve, gaussian jitter, time walk
#include <Riostream.h>
#include <TH1F.h>
#include <TRandom3.h>
#include <TCanvas.h>
...
void Coincidence(){
  ...
  TH2F* chi = new TH2F("chi","Distribuzione chi",176,3.75,9.25,144,6.75,11.25);	
  ...
  for(int i=0;i<29;i++){
    ...
    y_err[i] = TMath::Sqrt(counts[i]/(time[i]*time[i]) + rate_bk/70592. + err_cost*err_cost);
    if(-30<x[i] & x[i]<26){
      rate_m+=y[i];
      counter++;
    }
  ...
  double g1_width = 30;
  double rate = 1./(3*1.0197*352.81);
  double scarti_min = 10000;
  double scarti_l = 0;
  double scarti_h = 0;

  //Calcoliamo il chi quadro per ogni simulazione
  for(double h = 4.; h<9.; h+=0.03125){
    for(double l = 7.; l<11.; l+=0.03125){
      double g1_sigma = h;
      double adj = l;
      graph->Reset();
	...
	for(int j=0; j<2700; j++){
	  ...
	  if(temp<0 & temp+g1_width>=-gS_width+adj)
	    graph->Fill(x[i]);
	  if(temp>=0 & temp + att <=gS_width+adj)
	    graph->Fill(x[i]);
	}
      }
      graph->Scale(rate);
