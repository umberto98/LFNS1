...
//PIEDISTALLO
const int ped = 40; //channel -> da controllare
...
void ReadTree () {
    ...
    for(int ev=0;ev<tree->GetEntriesFast();ev++) {
        ...
        if (event.adc10<ped || event.adc11<ped) {
            cout<<" Canali NEGATIVI "<<ev<<endl;
        }
        hadc10->Fill(adc(event.adc10-ped)); //togliamo i canali del piedistallo
        if (event.pu==0) hadc11->Fill(adc(event.adc11-ped));
        else hadc11flag->Fill(adc(event.adc11-ped));
